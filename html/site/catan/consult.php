...
<html lang="en">
	<head>
		<title>Consult <?php echo $name; ?></title>
		<meta charset="utf-8">
	...
	<body>
		<div class="container-fluid mt-3">
			<h1 class="card-title">Consult <?php echo $name; ?></h1>
			<div class="row">
...
<script>
	...
	var god = {
		...
		busy_with: ";
			...
			$j = 0;
			echo "\t\toptions: [\n";
			$sql = "SELECT * FROM `catan_consult_options` WHERE `god_id`=$god_id";
			$result = $conn->query($sql);
			if($result->num_rows > 0) {
				while($row = $result->fetch_assoc()) {
					...
					echo "\t\t\t{\n\t\t\t\ttext: \"".$row['text']."\",
				responses: [\n";
					$sql = "SELECT * FROM `catan_responses` WHERE `consult_id`=".$row['ID'];
	...
	function clicked(index) {
		$("#responses").empty();
		var list = god.options[index].responses;
		var total = 1;
		var r = Math.random();
	...
	$(document).ready(function() {
		if(god.angry) {
			...
			$("#msg").html("What do you seek, my child?");
			fill_options(god.options);
		}
	});
</script>
